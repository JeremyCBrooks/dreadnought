...
def test_damaged_scanner_excluded_from_get_all_scanners():
    """A damaged scanner should not be returned by get_all_scanners."""
    damaged = Entity(
        name="Scanner",
        item={
            "type": "scanner",
    ...
    working = _scanner(tier=2)
    lo = Loadout(slot1=damaged, slot2=working)
    assert lo.get_all_scanners() == [working]
...
def test_scanner_does_not_add_melee_power():
    """An equipped scanner must NOT contribute to melee power."""
    player = Entity(name="Player", fighter=Fighter(10, 10, 0, 1))
    scanner = _scanner(tier=2)  # value=2
    player.loadout = Loadout(slot1=scanner)
    recalc_melee_power(player)
    assert player.fighter.power == player.fighter.base_power


def test_scanner_and_weapon_only_weapon_counts():
    """When a scanner and melee weapon are both equipped, only the weapon adds power."""
    player = Entity(name="Player", fighter=Fighter(10, 10, 0, 1))
    scanner = _scanner(tier=3)  # value=3
    weapon = Entity(
        name="Pipe",
        item={
            "type": "weapon",
    ...
    player.loadout = Loadout(slot1=scanner, slot2=weapon)
    recalc_melee_power(player)
