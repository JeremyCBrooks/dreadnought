...
class TestNavigationFocus:
    def test_direction_key_moves_to_connected_system(self):
        ...
        state = StrategicState(galaxy)
        engine = _make_engine(galaxy)
        state.ev_key(engine, FakeEvent(_sym("TAB")))  # switch to navigation
        state.ev_key(engine, FakeEvent(_sym("RIGHT")))
    ...
    def test_diagonal_navigation(self):
        ...
        state.ev_key(engine, FakeEvent(_sym("KP_3")))
...
class TestLabelPositioning:
    def test_labels_no_overlap_all_8_directions(self):
        ...
        console = SimpleNamespace(
            width=160,
            height=50,
            rgb=__import__("numpy").zeros(
                (160, 50), dtype=[("ch", __import__("numpy").int32), ("fg", "3u1"), ("bg", "3u1")]
            ),
        )
        console.print = mock_print
        console.draw_rect = lambda *a, **kw: None
        state.ev_key(engine, FakeEvent(_sym("TAB")))  # nav focus for bright labels
        ...
        for i, (x1, y1, s1) in enumerate(labels):
            for x2, y2, s2 in labels[i + 1 :]:
                ...
                assert not overlaps, f"Labels overlap on row {y1}: '{s1}' at x={x1}-{end1} vs '{s2}' at x={x2}-{end2}"
    ...
    def test_long_names_dont_overflow(self):
        ...
        for x, y, s in printed:
            if long_name in s or long_name[:20] in s:
                assert x + len(s) <= left_w, f"Label overflows: '{s}' at x={x} extends to {x + len(s)} (max {left_w})"
