...
def _make_creature(x, y, hp=3, max_hp=3, power=1, config=None, name="Rat"):
    ...
    e = Entity(
        ...
        name=name,
        char="r",
        fighter=Fighter(hp, max_hp, 0, power),
        ai=ai,
        organic=True,
    )
    e.ai_config = cfg
...
def _setup(w=20, h=20, player_pos=(1, 1), fov=True):
    gm = make_arena(w, h)
    player = Entity(x=player_pos[0], y=player_pos[1], name="Player", fighter=Fighter(10, 10, 0, 1))
...
class TestNoCheating:
    def test_hunting_moves_toward_last_known_not_real_pos(self):
        ...
        assert creature.x >= 10 or creature.y >= 10, "Should move toward last-known position, not real player"
...
class TestMoveSpeed:
    ...
    def test_slow_drone_cannot_keep_up_with_player(self):
        ...
        assert end_gap > start_gap, f"Speed-3 drone should fall behind player. Gap went from {start_gap} to {end_gap}"
    ...
    def test_normal_speed_creature_keeps_pace_with_player(self):
        ...
        for _ in range(20):
            ...
            creature.ai.perform(creature, engine)
        end_gap = chebyshev(creature.x, creature.y, player.x, player.y)
        assert end_gap == start_gap, f"Speed-4 creature should keep pace. Gap went from {start_gap} to {end_gap}"
    ...
    def test_attack_drains_energy(self):
        ...
        assert creature.ai_energy <= creature.ai_config["move_speed"], (
            f"Energy banked to {creature.ai_energy} during melee, expected at most {creature.ai_config['move_speed']}"
        )
    ...
    def test_slow_drone_falls_behind_after_melee(self):
        ...
        assert gap >= 3, f"Player should outrun speed-3 drone after melee, but gap is only {gap}"
    ...
    def test_energy_capped_prevents_teleport_on_state_change(self):
        ...
        assert distance_moved <= 2, f"Creature moved {distance_moved} steps, expected at most 2"
...
class TestDoorBlocking:
    ...
    def test_diagonal_past_wall_still_allowed(self):
        ...
        assert player.fighter.hp < 10, "Rat should be able to move diagonally past a wall corner"
...
class TestPatrolWander:
    ...
    def test_wandering_creature_picks_new_goal_on_arrival(self):
        ...
        creature.ai_wander_goal = (10, 10)
        creature.ai.perform(creature, engine)
        assert creature.ai_wander_goal != (10, 10), "Creature should pick a new goal after reaching the old one"
    ...
    def test_wandering_falls_back_to_hazard_when_all_hazardous(self):
        ...
        assert creature.ai_wander_goal is not None or (creature.x, creature.y) != (10, 10), (
...
class TestHuntingUnreachable:
    ...
    def test_multiple_enemies_unreachable_all_deaggro(self):
        ...
        for c in creatures:
            assert c.ai_state != "hunting", f"{c.name} still stuck in hunting after 30 turns"
...
# ---------------------------------------------------------------------------
# Bug fix: wandering→hunting must reset ai_stuck_turns
# ---------------------------------------------------------------------------


class TestSimpleChaseTracksMovement:
    def test_simple_chase_movement_resets_stuck_turns(self):
        """When pathfinding fails but _simple_chase moves the creature,
        ai_stuck_turns should reset to 0 (the creature IS making progress)."""
        gm, player, engine = _setup(w=20, h=20, player_pos=(5, 5))
        creature = _make_creature(
            15,
            5,
        ...
        creature.ai_state = "hunting"
        creature.ai_target = (5, 5)
        creature.ai_stuck_turns = 5  # some prior stuck turns
        gm.entities.append(creature)

        # Block pathfinding with a window wall but leave room to approach via simple_chase
        for y in range(0, 20):
            gm.tiles[10, y] = tile_types.structure_window

        creature.ai.perform(creature, engine)
        # Creature should have moved via simple_chase toward the player
        assert creature.x == 14, f"Expected creature at x=14, got x={creature.x}"
        assert creature.ai_stuck_turns == 0, (
            f"simple_chase movement should reset stuck_turns, got {creature.ai_stuck_turns}"
        )

    def test_stale_stuck_turns_cleared_by_movement_on_reaggro(self):
        """A creature that de-aggroed with high stuck_turns should clear them
        naturally by moving when re-aggroing in a reachable area."""
        gm, player, engine = _setup(w=20, h=20, player_pos=(5, 5))
        creature = _make_creature(
            8,
            5,
        ...
        creature.ai_state = "wandering"
        creature.ai_stuck_turns = 10  # stale from a prior stuck session
        gm.entities.append(creature)

        creature.ai.perform(creature, engine)
        # Should aggro and move toward player, resetting stuck_turns
        assert creature.ai_state == "hunting"
        assert creature.ai_stuck_turns == 0, f"Movement should reset stuck_turns, got {creature.ai_stuck_turns}"


# ---------------------------------------------------------------------------
# Bug fix: _do_fleeing must recompute distance after movement
# ---------------------------------------------------------------------------


class TestFleeingStaleDistance:
    def test_fleeing_no_ranged_attack_after_moving_away(self):
        """After fleeing away from the player, the creature should NOT
        fire a ranged attack using a stale distance check."""
        gm, player, engine = _setup(w=20, h=20, player_pos=(5, 5))
        creature = _make_creature(
            6,
            5,
        ...
        creature.inventory.append(weapon)
        ...
        dist = chebyshev(creature.x, creature.y, player.x, player.y)
        if dist > 1:
            assert player.fighter.hp == old_hp, "Fleeing creature should not attack after successfully moving away"


# ---------------------------------------------------------------------------
# Bug fix: cardinal-only fallback must validate adjacency
# ---------------------------------------------------------------------------


class TestCardinalFallbackAdjacency:
    def test_cardinal_fallback_returns_adjacent_step(self):
        """When _compute_path falls back to cardinal-only, the first step
        must still be adjacent to the creature."""
        gm, player, engine = _setup(w=10, h=10, player_pos=(1, 1))
        creature = _make_creature(5, 5, config={"aggro_distance": 15, "vision_radius": 15})
        gm.entities.append(creature)

        # Place a closed door diagonally to force the cardinal fallback
        gm.tiles[4, 4] = tile_types.door_closed

        ai = creature.ai
        path = ai._compute_path(creature, engine, (1, 1))
        if path:
            nx, ny = path[0]
            assert abs(nx - creature.x) <= 1 and abs(ny - creature.y) <= 1, (
                f"Cardinal fallback returned non-adjacent step ({nx},{ny}) from ({creature.x},{creature.y})"
            )
