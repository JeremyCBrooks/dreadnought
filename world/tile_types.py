...
# (char_code, (r, g, b) foreground, (r, g, b) background)
type Graphic = tuple[int, tuple[int, int, int], tuple[int, int, int]]
...
def _blend_graphic(dark: Graphic, light: Graphic, factor: float = 0.7) -> Graphic:
    ...
    ch = light[0]
    fg = tuple(int(d + (lv - d) * factor) for d, lv in zip(dark[1], light[1], strict=True))
    bg = tuple(int(d + (lv - d) * factor) for d, lv in zip(dark[2], light[2], strict=True))
    return (ch, fg, bg)  # type: ignore[return-value]
...
TILE_FLAVORS: dict[int, tuple[str, list[str]]] = {
    ...
    int(airlock_ext_closed["tile_id"]): (
    ...
    int(airlock_ext_open["tile_id"]): (
        "Exterior Airlock Door (Open)",
        [
    ...
    int(airlock_switch_off["tile_id"]): (
        "Airlock Switch (Off)",
        [
...
def describe_tile(tile_id: int, *, biome: str | None = None) -> tuple[str, str]:
    ...
    name, flavors = TILE_FLAVORS.get(tile_id, ("Unknown", ["You can't make out what this is."]))
    return name, _random.choice(flavors)
