...
def scatter_flora(
    ...
    weighted_zones = np.stack(
        [zone_fields[i] * palette.flora[i].density for i in range(n_types)], axis=-1
    )  # (w, h, n_types)
    ...
    # Place flora tiles
    for i, (_entry, tile) in enumerate(flora_tiles):
...
def apply_ground_noise(
    ...
    noise_channels = []
    for _ in range(3):
        field = fractal_noise(np_rng, w, h, octaves=3, base_radius=10)
